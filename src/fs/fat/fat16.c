...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector) {
    ...
    if(disk_streamer_seek(stream, directory_start_pos) != NO_ERROR) {
        res = -ERROR_IO;
        goto out;
    }

    while (1) {
        if (disk_stremer_read(stream, &item, sizeof(item)) != NO_ERROR) {
            res = -ERROR_IO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk,  struct fat_private *fat_private, struct fat_directory* directory) {
    ...
    int root_dir_size = root_dir_ennries * sizeof(struct fat_directory_item);
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (disk_stremer_read(stream, &fat_private->header, sizeof(fat_private->header)) != NO_ERROR) {
        res = -ERROR_IO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != NO_ERROR) {
        res = -ERROR_IO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
    ...
    // There is a bug in the next line : should be fat_table_position + (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE)
    res = disk_streamer_seek(stream, fat_table_position * (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0) {
        goto out;
    }

    uint16_t result = 0;
    res = disk_stremer_read(stream, &result, sizeof(result));
    if (res < 0) {
        goto out;
    }
...
static int fat16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out_buff) {
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.sectors_per_cluster * disk->sector_size;
    ...
    // Probably a bug in the next linge * inseted of +
    ...
    res = disk_stremer_read(stream, out_buff, total_to_read);
    if (res != NO_ERROR) {
        goto out;
    }
    ...
    if (total > 0) {
        // Have more to read
        res = fat16_read_internal_from_stream(disk, stream, cluster, offset + total_to_read, total, out_buff + total_to_read);
    }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* dir, const char* name) {
    ...
    for (int i = 0; i < dir->total; i++)
    {
        ...
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0) {
            // Found it. Creeate a new fat_item
