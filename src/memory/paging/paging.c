...
void* paging_align_address(void* ptr) {
    if (paging_is_aligned(ptr) == false) {
...
int paging_map_to(struct paging_4gb_chunk* directory, void* virt, void* phys, void* phys_end, int flags) {
    int res = 0;
    if (paging_is_aligned(virt) == false) {
        res = -ERROR_INVALID_ARG;
        goto out;
    }
    if (paging_is_aligned(phys) == false) {
        res = -ERROR_INVALID_ARG;
        goto out;
    }
    if (paging_is_aligned(phys_end) == false) {
        res = -ERROR_INVALID_ARG;
        goto out;
    }
...
/* Setting the mapping from virtual address to val which
Contains the physical address as well as flags */
...
// Get the physical address and flags entry (flags in lower 12 bits)
